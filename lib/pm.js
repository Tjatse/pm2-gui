...
pm.action = function(sockPath, action, id, cb){
  pm._findById(sockPath, id, function(err, proc){
    ...
    pm._actionByPMId(sockPath, proc.pm_id, action, cb);
  })
};
 ...
 * @private
 */
pm._actionByPMId = function(sockPath, pm_id, action, cb){
  action += 'ProcessId';
...
pm._tailLogs = function(proc, cb){
  ...
  if (proc.pm_log_path) {
    ...
    logs.push(['out', proc.pm2_env.pm_out_log_path]);
    logs.push(['err', proc.pm2_env.pm_err_log_path]);
  }
