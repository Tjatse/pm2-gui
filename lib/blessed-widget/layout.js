...
Layout.prototype.__proto__ = EventEmitter.prototype;
...
Layout.prototype._bindProcesses = function(){
  ...
  function setRows(forceRefresh){
    ...
    this._procs.data.forEach(function(p) {
      var pm2 = p.pm2_env;
      rows.push([
        ' ' + p.name,
        pm2.restart_time,
        pm2.status != 'online' ? '0s' : fromNow(Math.ceil((Date.now() - pm2.pm_uptime) / 1000), true),
