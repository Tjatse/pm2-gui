  ...
  async.series(Object.keys(conf.NSP).map(function (ns) {
    return function (next) {
      ...
      monitor.connect(_.extend({
        ...
        if (err) {
          return next(new Error('Fatal error leading to connect to ' + socket.nsp + ': ' + err))
        }
        next(null, socket)
      })
    }
...
Layout.prototype._observe = function () {
  ...
  this._socket(conf.NSP.PROCESS)
    .on(conf.SOCKET_EVENTS.DATA_PROCESSES, function (data) {
      if (data.processes) {
    ...
    .on(conf.SOCKET_EVENTS.DATA_USAGE, function (proc) {
      if (!self._data.usages || proc.id !== self._data.usages.id || self._data.usages.time === proc.time) {
        return
      }
...
Layout.prototype._processesTable = function () {
  ...
  if (_.isUndefined(this._data.processesLastTick)) {
    ...
    this._eles.processes.rows.on('select', _.debounce(this._onProcessesTableSelect.bind(this), 500))
  }
  // cache last tick
...
Layout.prototype._displayLogs = function (index) {
  ...
  this._stopLogging()
  this._socket(conf.NSP.LOG).emit(conf.SOCKET_EVENTS.PULL_LOGS, {
    id: pm2.pm_id,
    ansi: true
  })
